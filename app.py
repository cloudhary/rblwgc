...
def leaderboard():
    ...
    board = Match.query.join(User, User.id==Match.user_id).group_by(Match.user_id).add_column(func.count(Match.id)).add_column(User.name).order_by(func.count(Match.id).desc()).limit(50)
